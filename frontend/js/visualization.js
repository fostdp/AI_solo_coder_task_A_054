class LacquerVisualizer {
    ...
    updateMoistureColors() {
        if (!this.lacquerMesh) return;

        const colors = this.lacquerMesh.geometry.attributes.color;
    ...
    moistureToColor(moisture) {
        const clampedMoisture = Math.max(10, Math.min(90, moisture));
        const t = (clampedMoisture - 10) / 80;
    ...
    updateMarkerColors() {
        this.sensorMarkers.forEach(marker => {
            ...
            if (type === 'moisture') {
                ...
                value = this.moistureData[keys[index % keys.length]] || 50;
                ...
                value = this.strainData[keys[index % keys.length]] || 1;
